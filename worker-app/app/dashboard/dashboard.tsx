...
export default function WorkerDashboardScreen() {
  ...
  const getStatusBadge = (status: Task['status']) => {
    switch (status) {
      ...
      case 'pending':
        return (
          <View className="shrink-0 px-2.5 py-1 rounded-full bg-amber-100 border border-amber-200 dark:bg-amber-900/30 dark:border-amber-800">
      ...
      case 'completed':
        return (
          <View className="shrink-0 px-2.5 py-1 rounded-full bg-emerald-100 border border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800">
  ...
  const renderTaskCard = (task: Task) => {
    ...
    return (
      <TouchableOpacity
        key={task.id}
        className={`rounded-xl p-4 shadow-sm mb-4 active:scale-[0.98] ${
          isCompleted
            ? 'bg-white/60 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 opacity-75'
      ...
      >
        ...
        <View className="flex-row justify-between items-start mb-2">
          <Text
            className={`text-base font-semibold leading-tight pr-4 flex-1 ${
              isCompleted
        ...
        {!isCompleted && (
          <View className="flex-col gap-2 mb-0">
            <View className="flex-row items-center">
              ...
              />
              <Text className="text-slate-400 dark:text-slate-500 text-xs font-medium ml-1.5">
                {task.location}
              </Text>
            </View>
            <View className="flex-row items-center">
        ...
        {isCompleted && task.hasEvidence && (
          <View className="flex-row items-center">
            ...
            <Text className="text-emerald-500 dark:text-emerald-400 text-xs font-bold ml-1.5">
              Evidence Uploaded
            </Text>
          </View>
        )}
        ...
        {isInProgress && (
          <View className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-800 flex-row justify-between items-center">
            <View className="flex-row -space-x-2">
              {task.teamMember && (
                <View className="w-6 h-6 rounded-full bg-slate-200 border-2 border-white dark:border-slate-900 overflow-hidden">
                  <Image
  ...
  return (
    <View className="flex-1 bg-background-light dark:bg-background-dark">
      ...
      {/* Phone Container */}
      <View className="flex-1 max-w-[430px] w-full self-center bg-white dark:bg-background-dark shadow-2xl">
        {/* Status Bar Simulator */}
        <View className="h-12 w-full flex-row justify-between items-end px-8 pb-2">
        ...
        <View className="px-5 pt-4 pb-2">
          <View className="flex-row justify-between items-center mb-4">
            ...
            <View className="flex-row gap-2">
              <TouchableOpacity className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
        ...
        <View className="absolute bottom-0 left-0 right-0 h-20 bg-white/80 dark:bg-background-dark/80 border-t border-slate-200 dark:border-slate-800 flex-row items-center justify-around px-6 pb-4">
          <TouchableOpacity
          ...
          >
            <Icon
              name="grid-outline"
              size={24}
              color={activeTab === 'tasks' ? '#136dec' : '#94a3b8'}
            />
            <Text
              className={`text-[10px] font-bold ${
                activeTab === 'tasks' ? 'text-primary' : 'text-slate-400 dark:text-slate-500'
              }`}
            >
              Tasks
            </Text>
          ...
          >
            <Icon
              name="map-outline"
              size={24}
              color={activeTab === 'map' ? '#136dec' : '#94a3b8'}
            />
            <Text
              className={`text-[10px] font-medium ${
                activeTab === 'map' ? 'text-primary' : 'text-slate-400 dark:text-slate-500'
              }`}
            >
              Map
            </Text>
          ...
          >
            <Icon
              name="time-outline"
              size={24}
              color={activeTab === 'history' ? '#136dec' : '#94a3b8'}
            />
            <Text
              className={`text-[10px] font-medium ${
                activeTab === 'history' ? 'text-primary' : 'text-slate-400 dark:text-slate-500'
              }`}
            >
              History
            </Text>
          ...
          >
            <Icon
              ...
              size={24}
              color={activeTab === 'settings' ? '#136dec' : '#94a3b8'}
            />
            <Text
              className={`text-[10px] font-medium ${
                activeTab === 'settings' ? 'text-primary' : 'text-slate-400 dark:text-slate-500'
              }`}
            >
              Settings
            </Text>
        ...
        <View className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-32 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full" />
      </View>
    </View>
  );
}